 ...
 * @returns {{ displayName, informationDensity, timeHorizon, sensoryFlags, supportLevel, disorders, createdAt }}
 */
export function buildCAP(answers) {
  ...
  // sensoryFlags and disorders must always be arrays
  if (!Array.isArray(cap.sensoryFlags)) {
    cap.sensoryFlags = cap.sensoryFlags ? [cap.sensoryFlags] : []
  }
  if (!Array.isArray(cap.disorders)) {
    cap.disorders = cap.disorders ? [cap.disorders] : []
  }
