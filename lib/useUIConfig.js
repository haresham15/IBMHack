...
/** Write all theme tokens as CSS custom properties on <html> so every page inherits them */
function applyThemeToRoot(theme) {
    ...
    root.setProperty('--subtext', theme.subtext)
    root.setProperty('--font', theme.fontFamily)
    root.setProperty('--fz-body', theme.fontSize)
    root.setProperty('--fz-head', theme.fontSizeH)
    root.setProperty('--fz-small', theme.fontSizeS)
    root.setProperty('--lh', theme.lineHeight)
    root.setProperty('--min-tap', theme.largeTargets ? '52px' : '44px')
}
    ...
    useEffect(() => {
        ...
        fetch('/api/ui-config', {
            ...
            .then(data => {
                ...
                setTheme(t)
                applyThemeToRoot(t)   // ← global CSS vars on <html>
...
export function useStoredUIConfig() {
    ...
    useEffect(() => {
        try {
            ...
            if (stored) {
                const t = buildTheme(JSON.parse(stored))
                setTheme(t)
                applyThemeToRoot(t)  // ← ensure CSS vars are global on every page
            }
        } catch { /* ignore */ }
    }, [])
